...
internal static class SaveSysExFile
{
    ...
    public static string SaveAs(MT32State memorystate, string fileName = "New SysEx file.syx")
    {
        ...
        string? selectedFile = PlatformServices.FileDialog.ShowSaveFileDialog(
            "Save SysEx File",
            "MIDI System Exclusive message file|*.syx",
        ...
        if (ParseTools.IsNullOrWhiteSpace(selectedFile))
        {
            //user didn't select a file
            return FileTools.EMPTY;
        }
        return Save(memorystate, selectedFile, checkBeforeOverwriting: false);
    }
    ...
    public static string Save(MT32State memoryState, string fileName, bool checkBeforeOverwriting = true)
    {
        ...
        if (checkBeforeOverwriting && File.Exists(fileName) && !PlatformServices.Notification.AskUserToConfirm($"Overwrite {fileName}?", "MT-32 Editor"))
        { 
            return FileTools.CANCELLED;
        }
        ...
        {
            PlatformServices.Notification.ShowMessage($"Could not write SysEx file: {e.GetBaseException().Message}{Environment.NewLine}Please ensure you have write access to the selected folder path.", "MT-32 Editor");
        ...
        {
            PlatformServices.Notification.ShowMessage($"Saved SysEx data to {fileName}", "MT-32 Editor");
        }
        return fileName;
    ...
    public static void SaveSystemOnly(SystemLevel systemConfig, bool level, bool tuning, bool reverb, bool channels, bool reserve, bool messages)
    {
        string? selectedFile = PlatformServices.FileDialog.ShowSaveFileDialog(
            "Save SysEx File",
            "MIDI System Exclusive message file|*.syx",
            "New MT32 system settings file.syx");
        ...
        try
        {
            sysExFile = File.Create(selectedFile);
        }
        catch (Exception e)
        {
            PlatformServices.Notification.ShowMessage($"Could not write SysEx file: {e.GetBaseException().Message} {Environment.NewLine}Please ensure you have write access to the selected folder path.", "MT-32 Editor");
        ...
        sysExFile.Close();
        PlatformServices.Notification.ShowMessage($"Saved system settings to {selectedFile}", "MT-32 System Settings");
