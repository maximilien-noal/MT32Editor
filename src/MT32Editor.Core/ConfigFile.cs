...
internal static class ConfigFile
{
    ...
    public static string[] Load()
    {
        ...
        void CheckXPosSetting(string inputString)
        {
            ...
            if (xPos > 0 && xPos < PlatformServices.ScreenInfo.PrimaryScreenWidth - 30)
            {
                UISettings.WindowLocation[0] = xPos;
            }
        }
        ...
        void CheckYPosSetting(string inputString)
        {
            ...
            if (yPos > 0 && yPos < PlatformServices.ScreenInfo.PrimaryScreenHeight - 30)
            {
                UISettings.WindowLocation[1] = yPos;
            }
        }
        ...
        void CheckHeightSetting(string inputString)
        {
            ...
            if (height > 0)
            {
                UISettings.WindowSize[1] = height;
            }
        }
        ...
        void CheckWindowMaximisedSetting(bool? status)
        {
            if (status.HasValue)
            {
                UISettings.WindowMaximised = (bool)status;
            }
        }
        ...
        void CheckSaveWindowSizeSetting(bool? status)
        {
            if (status.HasValue)
            {
                UISettings.SaveWindowSizeAndPosition = (bool)status;
            }
        }
        ...
        void CheckDarkModeSetting(bool? status)
        {
            if (status.HasValue)
            {
                UISettings.DarkMode = (bool)status;
            }
        }
        ...
        void CheckTimbreEditorSetting(bool? status)
        {
            if (status.HasValue)
            {
                UISettings.PrioritiseTimbreEditor = (bool)status;
            }
        }
    }
    ...
    public static void Save()
    {
        try
        {
            ...
            fs.WriteLine($"{TEXT_SAVE_WINDOW_SIZE_POSITION} = {UISettings.SaveWindowSizeAndPosition}");
            if (UISettings.SaveWindowSizeAndPosition)
            {
                fs.WriteLine($"{TEXT_WINDOW_WIDTH} = {UISettings.WindowSize[0]}");
                fs.WriteLine($"{TEXT_WINDOW_HEIGHT} = {UISettings.WindowSize[1]}");
                fs.WriteLine($"{TEXT_WINDOW_X_POSITION} = {UISettings.WindowLocation[0]}");
                fs.WriteLine($"{TEXT_WINDOW_Y_POSITION} = {UISettings.WindowLocation[1]}");
                fs.WriteLine($"{TEXT_WINDOW_MAXIMISED} = {UISettings.WindowMaximised}");
            }
            fs.WriteLine($"{TEXT_AUTOSAVE} = {SaveSysExFile.autoSave}");
            fs.WriteLine($"{TEXT_DARK_MODE} = {UISettings.DarkMode}");
            ...
            fs.WriteLine($"{TEXT_PRIORITISE_TIMBRE_EDITOR} = {UISettings.PrioritiseTimbreEditor}");
            fs.Close();
        }
        catch (Exception)
        {
            PlatformServices.Notification.ShowMessage($"Unable to create {iniFileName}.{Environment.NewLine}{Environment.NewLine}Check that you have read/write access to the folder that the application is running from ({FileTools.applicationPath}){Environment.NewLine}{Environment.NewLine}MIDI device settings have not been saved.", "MT-32 Editor");
        }
    }
}
