...
internal static class TimbreFile
{
    ...
    {
        string? fileName = PlatformServices.FileDialog.ShowOpenFileDialog(
            "Load Timbre File",
            "Timbre file|*.timbre",
            checkFileExists: true);

        if (fileName is null || ParseTools.IsNullOrWhiteSpace(fileName))
        {
    ...
    public static string Load(TimbreStructure timbre, string fileName)
    {
        ...
        if (timbreFile.Length != VALID_FILE_LENGTH || Encoding.ASCII.GetString(header) != TIMBRE_FILE_HEADER)
        {
            ...
            timbreFile.Close();
            PlatformServices.Notification.ShowMessage($"{fileName} is not a valid timbre file.", "Unable to load timbre");
    ...
    /// <summary>
    /// Saves timbre data to file at the specified path
    /// </summary>

    public static void Save(TimbreStructure timbre, string fileName)
    {
        ...
        {
            PlatformServices.Notification.ShowMessage("Could not write timbre file. Please ensure you have write access to the selected folder path.");
        }
    }
    ...
    public static bool SaveAs(TimbreStructure timbre, string fileName)
    {
        ...
        if (selectedFile is null || ParseTools.IsNullOrWhiteSpace(selectedFile))
        {
            return false;
        }
        Save(timbre, selectedFile);
        return true;
    }

    public static bool QuickSave(TimbreStructure timbre, string currentFileName, string timbreName, bool allowQuickSave)
    {
        if (!allowQuickSave)
        {
            return SaveAs(timbre, timbreName);
        }

        string action = "Save";
        if (File.Exists(currentFileName)) action = "Overwrite";
        if (PlatformServices.Notification.AskUserToConfirm($"{action} file {currentFileName}?", "MT-32 Editor"))
        {
            Save(timbre, currentFileName);
            return true;
        }
        return false;
    }
    ...
    public static void SaveAll(TimbreStructure[] timbreArray)
    {
        ...
        if (fileCount == 0)
        {
            PlatformServices.Notification.ShowMessage("Couldn't save any timbre files- memory timbre slots are all empty.", "MT-32 Editor");
        }
        else
        {
            PlatformServices.Notification.ShowMessage($"{fileCount} timbre file{ParseTools.Pluralise(fileCount)} saved to {filePath}", "MT-32 Editor");
        }
