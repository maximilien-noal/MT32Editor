...
internal static class InstrumentDefinition
{
    ...
    public static string Save(MT32State memorystate, string fileName = "New Instrument Definition.ins")
    {
        ...
        return Populate(memorystate, selectedFile, checkBeforeOverwriting: false);
    }
    ...
    public static string Populate(MT32State memoryState, string fileName, bool checkBeforeOverwriting = true)
    {
        try
        {
            ...
            fs.WriteLine("");
            fs.Close();
            PlatformServices.Notification.ShowMessage($"Saved Instrument Definition to {fileName}", "MT-32 Editor");
            return fileName;
        }
        catch (Exception)
        {
            PlatformServices.Notification.ShowMessage($"Unable to create {fileName}.{Environment.NewLine}{Environment.NewLine}Check that you have read/write access to the folder that the application is running from ({FileTools.applicationPath}){Environment.NewLine}{Environment.NewLine}Instrument map has not been saved.", "MT-32 Editor");
