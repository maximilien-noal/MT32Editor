...
internal static class LoadSysExFile
{
    ...
    public static string Load(MT32State currentMemoryState)
    {
        string? fileName = PlatformServices.FileDialog.ShowOpenFileDialog(
            "Load SysEx File",
            "MIDI & SysEx files|*.syx; *.mid|SysEx files|*.syx|MIDI files|*.mid|All files|*.*",
    ...
    public static string Load(MT32State currentMemoryState, string fileName)
    {
        ...
        {
            PlatformServices.Notification.ShowMessage($"File '{Path.GetFileName(fileName)}' does not contain any valid MT-32 SysEx data.", "MT-32 Editor");
        ...
        try
        {
            bool result = PlatformServices.SysExLoadForm.ShowLoadDialog(memoryState, requestClearMemory: false);
            if (!result)
            {
