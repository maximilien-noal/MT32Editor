...
internal static class FileTools
{
    ...
    public static readonly string? applicationPath = AppContext.BaseDirectory?.TrimEnd(Path.DirectorySeparatorChar, Path.AltDirectorySeparatorChar);
    ...
    {
        string? pathName = PlatformServices.FileDialog.ShowFolderBrowserDialog();
        if (pathName is null)
        {
            return ERROR;
        }
        if (ParseTools.IsNullOrWhiteSpace(pathName))
        {
            return CANCELLED;
        }
        return pathName;
